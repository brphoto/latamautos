...
class HrFamily(models.Model):
    ...
    def _compute_age(self):
        for row in self:
            if row.birthday:
                date_birthday = datetime.strptime(str(row.birthday), "%Y-%m-%d")
                if date_birthday <= datetime.today():
                    today = datetime.today().strftime("%Y-%m-%d")
                    now = today.split("-")
                    birth = row.birthday.strftime("%Y-%m-%d").split("-")
            ...
            if row.date_marriage:
                date_marriage = datetime.strptime(str(row.date_marriage), "%Y-%m-%d")
                if date_marriage <= datetime.today():
                    today = datetime.today().strftime("%Y-%m-%d")
                    now = today.split("-")
                    marriage = row.date_marriage.strftime("%Y-%m-%d").split("-")
                    date_now = date(int(now[0]), int(now[1]), int(now[2]))
                    date_marriage = date(
                        int(marriage[0]), int(marriage[1]), int(marriage[2])
                    )
    ...
    years_marriage = fields.Integer(
        compute=_compute_age, string=_("Years Of Marriage"), store=True
    )
